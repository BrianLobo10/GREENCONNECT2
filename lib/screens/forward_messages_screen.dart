...
class _ForwardMessagesScreenState extends State<ForwardMessagesScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Reenviar ${widget.messages.length} mensaje${widget.messages.length > 1 ? 's' : ''}',
  ...
  Widget _buildBody() {
    ...
    return Column(
      children: [
        // Vista previa de los mensajes a reenviar
        Container(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      ...
                      child: Text(
                        '${index + 1}. ${message.texto.length > 30 ? '${message.texto.substring(0, 30)}...' : message.texto}',
  ...
  void _forwardToUser(User user) async {
    try {
      ...
      // Reenviar cada mensaje en orden
      for (final message in widget.messages) {
        await messagesProvider.forwardMessage(message, user.id!);
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${widget.messages.length} mensaje${widget.messages.length > 1 ? 's' : ''} reenviado${widget.messages.length > 1 ? 's' : ''} a ${user.nombre}'),
